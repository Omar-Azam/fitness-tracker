    ...
    <div ref={containerRef} className="relative w-full max-w-xs sm:max-w-sm">
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-8 py-1.5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 transition"
        />
        {loading ? (
      ...
      {/* Results Dropdown */}
      {isOpen && query.trim() && (
        <div className="absolute top-full mt-2 left-0 right-0 z-50 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden max-h-[75vh] overflow-y-auto">
          {!hasResults && !loading ? (
            ...
            <div className="divide-y divide-slate-800">
              ...
              {results.workouts.length > 0 && (
                <div className="p-3">
                  <div className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-emerald-400 px-2 pb-2">
                  ...
                  <div className="space-y-1">
                    {results.workouts.map((w) => (
                      <button
                        ...
                        className="w-full text-left p-2 rounded-xl hover:bg-slate-800/80 transition flex items-center justify-between group cursor-pointer"
                      >
                        <div className="truncate pr-2">
                          <p className="text-xs font-semibold text-white group-hover:text-emerald-400 transition truncate">
                            {w.name}
                          </p>
                        ...
                        <ChevronRight className="h-3.5 w-3.5 text-slate-500 group-hover:text-emerald-400 transition shrink-0" />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {results.nutrition.length > 0 && (
                <div className="p-3">
                  <div className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-amber-400 px-2 pb-2">
                        ...
                        >
                          <div className="truncate pr-2">
                            <div className="flex items-center gap-1.5">
                              <span className="text-xs font-semibold text-white group-hover:text-amber-400 transition capitalize">
                                {n.mealType}
                              </span>
                          ...
                          <ChevronRight className="h-3.5 w-3.5 text-slate-500 group-hover:text-amber-400 transition shrink-0" />
                        </button>
                      );
                    })}
                  </div>
