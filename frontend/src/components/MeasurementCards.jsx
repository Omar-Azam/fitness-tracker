    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
      {MEASUREMENT_CONFIG.map(({ key, label, isTopLevel }) => {
        ...
        return (
          <div
            key={key}
            className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-lg flex flex-col justify-between space-y-2 hover:border-slate-700 transition"
          >
            ...
            <div>
              ...
              <div className="mt-1.5 flex items-center gap-1 text-[11px] font-mono font-medium">
                {pctChange === null ? (
                  <span className="text-slate-500 text-[10px]">No baseline</span>
                ) : pctChange === 0 ? (
                  ...
                  <span className="inline-flex items-center text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/20">
